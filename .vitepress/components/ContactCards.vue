<script setup>
// `accent` carries a light and a dark shade: the pale blues that read well on
// the dark page disappear into a white one.
const channels = [
  {
  ...
  <div class="my-8 grid gap-6 md:grid-cols-3">
    ...
    >
      <div class="mb-6 w-fit rounded-2xl border border-line bg-fill p-3.5">
        ...
        <svg v-else-if="c.icon === 'mail'" width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
      ...
      <h3 class="mb-2 font-display text-lg font-semibold text-heading">{{ c.title }}</h3>
      <p class="text-sm leading-relaxed text-muted">{{ c.desc }}</p>
    </a>
  </div>
</template>
