...
export default function Header() {
  ...
  return (
    // One frosted navbar for every page: sticky, so it stays pinned at
    // the top while the page scrolls under the glass. The negative
    // bottom margin cancels its own height (pt-4 + h-14 = 4.5rem) so
    // the bar floats over the page exactly like a fixed header and no
    // page's content shifts. Nav links sit in the centre, Apple-style.
    <div className="sticky top-0 z-50 -mb-[4.5rem] w-full px-4 pt-4 sm:px-6">
      <motion.header
        initial={{ opacity: 0, y: -20, filter: 'blur(8px)' }}
        animate={{ opacity: 1, y: 0, filter: 'blur(0px)' }}
        transition={{ duration: 0.7, ease: [0.16, 1, 0.3, 1] }}
        className="relative mx-auto flex h-14 w-full max-w-5xl items-center justify-between rounded-2xl border border-white/10 bg-white/[0.07] px-5 shadow-[0_8px_32px_rgba(0,0,0,0.35)] backdrop-blur-2xl"
      >
        ...
        <NavItems className="absolute left-1/2 hidden -translate-x-1/2 items-center gap-8 md:flex" />
      ...
      {/* Mobile menu - absolutely positioned below the bar so it overlays
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="absolute left-4 right-4 top-full mx-auto mt-2 max-w-5xl overflow-hidden rounded-2xl border border-white/10 bg-[#121215]/90 backdrop-blur-2xl md:hidden sm:left-6 sm:right-6"
          >
